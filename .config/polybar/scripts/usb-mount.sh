...
mountusb(){ \
    ...
    mountpoint=$(udisksctl mount --no-user-interaction -b "$chosen" 2>/dev/null) && notify-send "  mounting USB" "$chosen mounted to $mountpoint" && exit 0
...
umountusb(){ \
    ...
    mountpoint=$(udisksctl unmount --no-user-interaction -b "$chosen" 2>/dev/null) && notify-send "  unmounting USB" "$chosen mounted" && exit 0
